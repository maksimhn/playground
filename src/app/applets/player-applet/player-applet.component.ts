...
export class PlayerAppletComponent implements AfterViewInit, OnInit {
  urlBase: string;
  audioTracksList: Object[];
  intro: string;
  ...
  @Input() selectableButton: boolean = true;
  ...
  constructor(_staticTextService: StaticTextService, _dynamicDataService: DynamicDataService) {
    ...
    _dynamicDataService.getMusicTracks()
      .then((response) => {
        this.audioTracksList = response.Contents;
      })
      .then(() => {
        if (this.audioTracksList.length) {
          this.audioTracksList.map((el) => {
            this.src.push(this.urlBase + '/' + el['Key']);
          });
        }
      })
      .then(() => {
  ...
  playThis(songKey: string): void {
    if (songKey) {
      this.player.nativeElement.src = this.src[ this.src.indexOf(this.urlBase + '/' + songKey) ];
    }
  }
  ...
  setIncrementInterval(interval: number): any {
    return setInterval(() => {
      ...
      this.player.nativeElement.volume += (this.player.nativeElement.volume * 100) / 100;
